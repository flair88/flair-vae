...
function get_items( $filter ) {
	...
	if( !is_null($_REQUEST['id']) ) {
		$id = $_REQUEST['id'];
		$cat = vae('/shop/categories[hide!=1]/' . $id);

		if( $cat->title == 'All' ) {

			// all items
			$layout = 'all';
			$items = vae('/shop/items[inventory>0]', $filter);

		} elseif ($cat->id == 584675){
            $items = vae('/gift_cards/cards[inventory>0]', $filter);
    }else {

			// category / subcategory
			$items = vae( $xpath, $filter);
		...
		// shop.php
		$items = vae('/shop/items[inventory>0][featured=1]', $filter);
	}
